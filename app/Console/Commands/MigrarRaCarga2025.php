...
#[Signature('migrar:ra-carga2025 {--etl-id= : ID en migraciones_etl para reportar progreso}')]
#[Description('Migra las tablas de ra_carga2025 (nacion) al esquema ra_carga2025 en planeamiento')]
class MigrarRaCarga2025 extends Command
{
    private const SCHEMA  = 'ra_carga2025';
    ...
    public function handle(): int
    {
        ...
        try {
            ...
            $this->info('✓ Migración ra_carga2025 completada.');
            return self::SUCCESS;
    ...
    {
        $total = DB::connection('nacion')->table('localizacion')->count();
        ...
        DB::connection('nacion')
            ...
            ->chunk(self::CHUNK, function ($filas) use (&$procesados, $total, $etl, $bar, $porcentajeBase) {
                ...
                DB::table(self::SCHEMA . '.localizacion')->insert($datos);
    ...
    private function migrarDetalleCuadro(?MigracionEtl $etl, int $porcentajeBase): void
    {
        ...
        $this->procesarQueryCompleta($sql, self::SCHEMA . '.detalle_cuadro',
            fn ($f) => [
                'c_oferta'                  => $f->c_oferta,
    ...
    private function migrarEstadoCargaNivel(?MigracionEtl $etl, int $porcentajeBase): void
    {
        ...
        $this->procesarQueryCompleta($sql, self::SCHEMA . '.estado_carga_nivel',
            fn ($f) => [
                'id_localizacion'     => $f->id_localizacion,
    ...
    private function migrarEstadoCarga(?MigracionEtl $etl, int $porcentajeBase): void
    {
        ...
        $this->procesarQueryCompleta($sql, self::SCHEMA . '.estado_carga',
            fn ($f) => [
                'id_localizacion' => $f->id_localizacion,
                'cuadernillo'     => $f->cuadernillo,
                'estado'          => $f->estado,
    ...
    private function migrarCuadrosDinamicos(?MigracionEtl $etl, int $porcentajeBase): void
    {
        ...
        $cuadros = DB::connection('nacion')->select("
    ...
    private function procesarCuadro(object $cuadro): void
{
    ...
    $columnas = DB::connection('nacion')->select("
        SELECT
            definicion_columna.nombre_corto AS columna,
        ...
        ORDER BY defcuadro_defcolumna.orden ASC
    ", [$idCuadro]);

    $columnas = array_filter(
        $columnas,
        fn ($c) => trim(str_replace(' ', '', $c->columna)) !== ''
    );
    ...
    // Crear tabla destino con nombres sanitizados
    ...
    // Query pivotada: nombre ORIGINAL en el CASE (lee de nacion),
    // nombre SANITIZADO como alias (escribe en planeamiento)
    ...
    $sqlDatos = "
        ...
        FROM (
            SELECT
                loc.id_localizacion,
                def_col.nombre_corto AS columna,
    ...
    $filas = DB::connection('nacion')->select($sqlDatos);

    if (empty($filas)) {
        return;
    }

    // Usar nombres sanitizados para mapear las claves del objeto
    $columnaKeys = array_map(
        fn ($c) => $this->sanitizarColumna($c->columna),
        $columnas
    );
        ...
        ?MigracionEtl $etl, int $porcentajeBase): void
    {
        ...
        $filas = DB::connection('nacion')->select($sql);
        $total = count($filas);
